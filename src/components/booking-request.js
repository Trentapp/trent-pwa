...
const BookingRequest = props => {
    return(
        ...
        >
            ...
            <Modal.Body>
                <p>{t("Specify time to borrow here")}</p>
                ...
                <b>Achtung: Die Option für Online-Payment ist noch nicht vorhanden, kommt aber bald. Mit dem Click auf {t("Send Request")} stimmen sie zu den angezeigten Preis an <em>{props.product.user.name}</em> zu zahlen. Standardmäßig mit Bar. (Sie können die Transaktion vor dem Termin noch abbrechen)</b>
            </Modal.Body>
            <Modal.Footer>
                <Button onClick={props.onSendRequest}>{t("Send Request")}</Button>
            </Modal.Footer>
        </Modal>
    )
}
