            ...
            <Box p={4}>
                <Center>
                    <VStack spacing="2px">
                        ...
                        <Box w="100%" py={3}>
                            <HStack>
                                <Link to={`/profile/${props.user._id}`}><Button variant="outline">{t("Your Profile")}</Button></Link>
                                <Link to="/chats"><Button variant="outline">{t("Chats")}</Button></Link>
                                <Link to="/transactions"><Button variant="outline">{t("Transactions")}</Button></Link>
                            </HStack>
                        </Box>
                        <Divider color="gray.400"/>
                        <VStack align="flex-start" paddingTop={3}>
                            {newMessagesChats.length && <Box paddingTop={3}>
                                <Heading size="lg">{t("Chats")} with new messages</Heading>
                                <VStack align="flex-start">
                                {newMessagesChats.map(chat => <Link to={`/chats/${chat._id}`}><Box w="100%" p={2} border="1px" borderRadius="lg" borderColor="gray.400" key={chat._id}>{props.user._id === chat.borrower._id ? <>{chat.lender.name}{t(" lending ")}{chat.product?.name}</> : <>{chat.borrower.name}{t(" borrowing your ")}{chat.product?.name}</>}</Box></Link> )}
                                </VStack>
                            </Box>}
                            {newRequests.length && <Box paddingTop={3}>
                                <Heading size="lg">{t("New Requests")}</Heading>
                                <VStack align="flex-start">
                            ...
                            {upcomingTransactions.length && <Box paddingTop={3} paddingBottom={3}>
                                <Heading size="lg">{t("Upcoming transactions")}</Heading>
                                <VStack align="flex-start">
